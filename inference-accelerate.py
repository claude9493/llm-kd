...
# if 'id' not in test_data.column_names:
#     test_data = test_data.add_column('_id', list(range(test_data.num_rows)))
...
for _, (_ids, data) in tqdm(enumerate(dataloader), total=len(dataloader)):
    with torch.no_grad():
        out = unwrap_model(model).generate(**data, generation_config=generation_config).sequences
    # print(out)
    results.extend([{"id":_id, label_column: summary} for _id, summary in zip(_ids, tokenizer.batch_decode(out))])
